...
// 地图加载完成回调
// 详情页「地图查看/导航」按钮 → 携带 navLng/navLat 跳回首页，此处消费并定位（前端B）
const pendingNav =
  route.query.navLng != null && route.query.navLat != null
    ? { longitude: Number(route.query.navLng), latitude: Number(route.query.navLat) }
    : null

function onMapReady(olMap) {
  // 有导航请求 → 优先定位到目标点并滚动到地图；否则自适应标记点范围
  if (pendingNav && mapRef.value) {
    mapRef.value.setCenter(pendingNav.longitude, pendingNav.latitude, 16)
